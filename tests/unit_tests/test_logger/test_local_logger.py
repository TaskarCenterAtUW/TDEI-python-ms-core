...
class TestLocalLogger(unittest.TestCase):
    @patch('src.python_ms_core.core.queue.local_queue')
    def setUp(self, MockLocalQueue):
        self.config = Mock()
        self.queue_client = MockLocalQueue.return_value
        self.logger = LocalLogger(config=self.config)
